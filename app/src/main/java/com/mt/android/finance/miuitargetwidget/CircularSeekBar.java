...
public class CircularSeekBar extends View {
	...
	private static final float DEFAULT_POINTER_RADIUS = 5f;// 7f圆圈的半径
	private static final float DEFAULT_POINTER_HALO_WIDTH = 2f;// 6f圆圈颜色的粗细
	private static final float DEFAULT_POINTER_HALO_BORDER_WIDTH = 2f;
	private static final float DEFAULT_CIRCLE_STROKE_WIDTH = 2f;// 5f大圈颜色的粗细
	...
	private static final int DEFAULT_CIRCLE_COLOR = 0xFF429DF9;
	// private static final int DEFAULT_CIRCLE_PROGRESS_COLOR = Color.argb(235,
	// 74, 138, 255);
	private static final int DEFAULT_CIRCLE_PROGRESS_COLOR = Color.argb(255,
			24, 255, 126);
	private static final int DEFAULT_POINTER_COLOR = Color.argb(235, 74, 138,
			255);
	private static final int DEFAULT_POINTER_HALO_COLOR = Color.argb(135, 74,
			138, 255);
	private static final int DEFAULT_POINTER_HALO_COLOR_ONTOUCH = Color.argb(
			135, 255, 255, 255);
	private static final int DEFAULT_CIRCLE_FILL_COLOR = Color.TRANSPARENT;// 内部颜色
	...
	private void initAttributes(TypedArray attrArray) {
		...
		tempColor = attrArray
				.getString(R.styleable.CircularSeekBar_pointer_halo_color_ontouch);
		if (tempColor != null) {
			try {
		...
		tempColor = attrArray
				.getString(R.styleable.CircularSeekBar_circle_color);
		if (tempColor != null) {
			try {
		...
		tempColor = attrArray
				.getString(R.styleable.CircularSeekBar_circle_progress_color);
		if (tempColor != null) {
			try {
		...
		tempColor = attrArray
				.getString(R.styleable.CircularSeekBar_circle_fill);
		if (tempColor != null) {
			try {
	...
	public boolean onTouchEvent(MotionEvent event) {
		...
		//
		// // Get the distance from the center of the circle in terms of a
		// radius
		...
		//
		// if (mCircleStrokeWidth < minimumTouchTarget) { // If the width is
		// less
		// // than the
		// // minimumTouchTarget,
		// // use the
